...
export class MulterMiddleware {
  ...
  uploadProfile = multer({ storage: this.storage }).single('file');
  ...
  memoryStorageProfile = (req: Request, res: Response, next: NextFunction) => {
    try {
      // Use the multer middleware to process the upload
      this.uploadProfile(req, res, (err: any) => {
        if (err) {
          // Handle any upload errors here
          return res.status(400).json({ error: 'File upload failed' });
        }
